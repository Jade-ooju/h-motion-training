...
# 정확도 비교
if val_accuracy >= rf_accuracy * 0.95:  # 95% 이상 유지
...
try:
    ...
    mlp_keras.layers[3].set_weights([mlp.coefs_[3].T, mlp.intercepts_[3]])
    print("✅ 가중치 복사 완료!")
    
except Exception as e:
    print(f"⚠️ 가중치 복사 실패: {e}")
    print("🔧 랜덤 가중치로 초기화하고 훈련을 진행합니다...")
    ...
    # 훈련 후 성능 확인
    keras_accuracy = mlp_keras.evaluate(X_val, y_val, verbose=0)[1]
    print(f"📊 Keras 모델 정확도: {keras_accuracy:.4f}")

# --- 7. 모델 저장 ---
print("\n💾 모델 저장 중...")

# Keras 모델 저장
mlp_keras.save("final_mlp_motion_classifier.h5")
print("✅ Keras 모델을 'final_mlp_motion_classifier.h5'로 저장했습니다.")
...
try:
    ...
    with open("final_mlp_motion_classifier.onnx", "wb") as f:
        f.write(model_proto.SerializeToString())
    
    print("✅ ONNX 모델을 'final_mlp_motion_classifier.onnx'로 저장했습니다.")
    ...
    # SavedModel로 저장
    mlp_keras.save("final_mlp_saved_model", save_format='tf')
    print("✅ SavedModel을 'final_mlp_saved_model'로 저장했습니다.")
...
try:
    ...
    ort_session = ort.InferenceSession("final_mlp_motion_classifier.onnx")
...
{{
    ...
    {{
        return $"MLP 모션 분류기\\n" +
               ...
               $"MLP 훈련 정확도: {train_accuracy:.1%}\\n" +
               $"MLP 검증 정확도: {val_accuracy:.1%}";
    }}
}}
...
with open("unity_final_mlp_integration.cs", "w", encoding="utf-8") as f:
    f.write(unity_code)

print("✅ Unity 통합 코드를 'unity_final_mlp_integration.cs'로 저장했습니다.")

# --- 11. 요약 ---
print("\n" + "=" * 50)
print("🎉 최종 MLP → ONNX 변환 완료!")
print("=" * 50)
print(f"🏆 원본 Random Forest 정확도: {rf_accuracy:.4f}")
print(f"🌿 MLP 훈련 정확도: {train_accuracy:.4f}")
print(f"🌿 MLP 검증 정확도: {val_accuracy:.4f}")
print(f"📊 입력 특징 수: {len(selected_features)}")
print(f"💾 생성된 파일들:")
print(f"   - final_mlp_motion_classifier.h5 (Keras 모델)")
print(f"   - final_mlp_motion_classifier.onnx (ONNX 모델)")
print(f"   - unity_final_mlp_integration.cs (Unity 통합 코드)")
print("\n🚀 이제 Unity에서 ONNX 모델을 사용하세요!")
print("💡 ONNX Runtime for Unity를 설치하고 위의 C# 코드를 사용하세요!")
print(f"🎯 MLP가 Random Forest 성능의 {val_accuracy/rf_accuracy*100:.1f}%를 유지합니다!")
