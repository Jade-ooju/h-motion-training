...
# 성능 평가
lr_accuracy = lr_model.score(X_selected, y)
...
try:
    # 로지스틱 회귀 가중치를 TensorFlow에 직접 복사
    tf_model.layers[0].set_weights([
        lr_model.coef_.T,  # 가중치 (특징 수 x 클래스 수)
        lr_model.intercept_  # 편향
    ])
    print("✅ 가중치 복사 완료!")
    
    # 복사 후 성능 확인
    tf_prediction = tf_model.predict(test_sample)
    tf_proba = tf_prediction[0]
    tf_pred_class = np.argmax(tf_proba)
    
    print(f"🎯 TensorFlow 예측: {class_names[tf_pred_class]}")
    print(f"📊 TensorFlow 확률: {tf_proba}")
    
    # 예측 일치성 확인
    if tf_pred_class == rf_prediction[0]:
        print("✅ TensorFlow 모델이 Random Forest와 예측이 일치합니다!")
    else:
        print("⚠️ TensorFlow 모델이 Random Forest와 예측이 다릅니다.")
    ...
    # 훈련 후 성능 확인
    tf_accuracy = tf_model.evaluate(X_selected, y, verbose=0)[1]
...
# TensorFlow 모델 저장
tf_model.save("final_simple_rf_equivalent.h5")
print("✅ TensorFlow 모델을 'final_simple_rf_equivalent.h5'로 저장했습니다.")
...
try:
    ...
    with open("final_simple_rf_equivalent.onnx", "wb") as f:
        f.write(model_proto.SerializeToString())
    
    print("✅ ONNX 모델을 'final_simple_rf_equivalent.onnx'로 저장했습니다.")
    ...
    # SavedModel로 저장
    tf_model.save("final_simple_rf_equivalent_saved_model", save_format='tf')
    print("✅ SavedModel을 'final_simple_rf_equivalent_saved_model'로 저장했습니다.")
...
try:
    ...
    ort_session = ort.InferenceSession("final_simple_rf_equivalent.onnx")
    ...
    # TensorFlow와 비교
    if onnx_prediction[0] == tf_pred_class:
        print("✅ ONNX 모델이 TensorFlow 모델과 예측이 일치합니다!")
    else:
        print("⚠️ ONNX 모델이 TensorFlow 모델과 예측이 다릅니다.")
...
unity_code = f"""
// Unity C# 최종 간단한 Random Forest 대체 모델 통합 코드
...
public class FinalSimpleRFMotionClassifier
{{
    ...
    {{
        return $"최종 간단한 Random Forest 대체 모델\\n" +
...
// float[] inputFeatures = FinalSimpleRFMotionClassifier.CompressSequence(allJointPositions, allJointRotations);
// string predictedMotion = FinalSimpleRFMotionClassifier.ClassifyMotion(inputFeatures);
"""

with open("unity_final_simple_rf_integration.cs", "w", encoding="utf-8") as f:
    f.write(unity_code)

print("✅ Unity 통합 코드를 'unity_final_simple_rf_integration.cs'로 저장했습니다.")

# --- 11. 요약 ---
print("\n" + "=" * 50)
print("🎉 최종 간단한 Random Forest 대체 모델 → ONNX 변환 완료!")
print("=" * 50)
print(f"🏆 원본 Random Forest 정확도: {rf_accuracy:.4f}")
print(f"🔧 대체 모델 정확도: {lr_accuracy:.4f}")
print(f"📊 입력 특징 수: {len(selected_features)}")
print(f"💾 생성된 파일들:")
print(f"   - final_simple_rf_equivalent.h5 (TensorFlow 모델)")
print(f"   - final_simple_rf_equivalent.onnx (ONNX 모델)")
print(f"   - unity_final_simple_rf_integration.cs (Unity 통합 코드)")
...
# --- 12. 추가 정보 ---
print("\n💡 추가 정보:")
print("🔍 StandardScaler 없이 로지스틱 회귀를 훈련하여 예측 일치성을 높였습니다.")
print("🎯 이 방법은 데이터 정규화로 인한 예측 변화를 방지합니다.")
print("🚀 Unity에서 실시간 동작 분류가 가능합니다!")
